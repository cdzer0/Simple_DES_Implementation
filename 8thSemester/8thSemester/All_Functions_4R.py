...
def keyRotationNpermuteTwo(k,i,retdict,eord):
    round_shift_encrypt = [1,1,2,2,2,2,2,2]
    round_shift_decrypt = [14,2,2,2,2,2,2,1]
    l_k = ""
    r_k = ""
    lcs_key = ""
    permutation = ""
    if eord == 'e':
        for m in range(round_shift_encrypt[i],28):
            l_k += k[m]
        for n in range(0,round_shift_encrypt[i]):
            l_k += k[n]
        #print("l_k for " + str(i + 1) + " is " + l_k)
        for m in range(28+round_shift_encrypt[i],56):
            r_k += k[m]
        for n in range(28,28+round_shift_encrypt[i]):
            r_k += k[n]
        #print("r_k for " + str(i + 1) + " is " + r_k)
    elif eord == 'd':
        for m in range(round_shift_decrypt[i],28):
            l_k += k[m]
        for n in range(0,round_shift_decrypt[i]):
            l_k += k[n]
        #print("l_k for " + str(i + 1) + " is " + l_k)
        for m in range(28+round_shift_decrypt[i],56):
            r_k += k[m]
        for n in range(28,28+round_shift_decrypt[i]):
            r_k += k[n]
    for l in range(56):
        if l<28:
            lcs_key += l_k[l]
